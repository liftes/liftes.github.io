...
function bibOrder(a, b) { return (a.sortkey || '').localeCompare(b.sortkey || '') || works.indexOf(a) - works.indexOf(b); }
function assignCitationIds() { ['mainwork', 'collaborative'].forEach((group, index) => { works.filter(work => group === 'mainwork' ? work.keywords.includes('mainwork') : !work.keywords.includes('mainwork')).sort(bibOrder).forEach((work, number) => { work.citationId = `${index === 0 ? 'M' : 'C'}${number + 1}`; work.citationRank = number + 1; }); }); }
